...
typedef struct
{
  ...
  uint8 buff[(ETH_RX_BUFFER_SIZE + 31) & ~31] __ALIGNED(32);
} RxBuff_t;
...
int32_t ETH_PHY_IO_ReadReg(uint32 DevAddr, uint32 RegAddr, uint32 *pRegVal);
int32_t ETH_PHY_IO_WriteReg(uint32 DevAddr, uint32 RegAddr, uint32 RegVal);
...
{
  uint32 duplex, speed = 0U;
  int32_t PHYLinkState = 0U;
  ETH_MACConfigTypeDef MACConf = {0};
  uint8 macaddress[6]= {ETH_MAC_ADDR0, ETH_MAC_ADDR1, ETH_MAC_ADDR2, ETH_MAC_ADDR3, ETH_MAC_ADDR4, ETH_MAC_ADDR5};
...
void ethernet_link_thread( void const * argument )
{
  ETH_MACConfigTypeDef MACConf = {0};
  int32_t PHYLinkState = 0U;
  uint32 linkchanged = 0U, speed = 0U, duplex = 0U;
  struct netif *netif = (struct netif *) argument;
...
void HAL_ETH_RxAllocateCallback(uint8 **buff)
{
  ...
  if (p)
  {
    ...
    *buff = (uint8 *)p + offsetof(RxBuff_t, buff);
...
void HAL_ETH_RxLinkCallback(void **pStart, void **pEnd, uint8 *buff, uint16 Length)
{
  ...
  *ppEnd  = p;
  ...
  SCB_InvalidateDCache_by_Addr((uint32 *)buff, Length);
}

void HAL_ETH_TxFreeCallback(uint32 * buff)
{
